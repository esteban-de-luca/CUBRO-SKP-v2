...
def calcular_opciones(entrada: list[dict]) -> list[dict]:
    ...
    for i, fila in enumerate(entrada):
        ...
        _fondo_csv   = (fila.get("Fondo CSV") or "").replace("mm", "").strip()
        ...
        _tiene_fondo_var = _cat_e_dim.get("fondo_variable") is not None
        _codigos_encimera: set[str] = set((op_mueble.get("encimeras") or {}).get("codigos") or [])
        ...
        if code == "FF12V":
            ...
            _p_depth  = 0
        elif code in _codigos_encimera:
            # Encimeras: p_width=ancho, p_depth=fondo (ambos variables), p_height=espesor (fijo)
            _p_width  = int(_ancho_csv) if _ancho_csv.isdigit() else 0
            _p_height = int(_cat_e_dim["alto_mm"]) if _cat_e_dim.get("alto_mm") is not None else 0
            _p_depth  = int(_fondo_csv) if _tiene_fondo_var and _fondo_csv.isdigit() else 0
        else:
