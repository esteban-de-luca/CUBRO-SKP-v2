...
def _control_op_126(
    ...
    """Bloque electro: radio Caso A/B + campos condicionales."""
    if not isinstance(opcionales.get("op_126"), dict):
        opcionales["op_126"] = {}
    datos: dict = opcionales["op_126"]
    caso_actual = datos.get("caso", "A")

    caso = st.radio(
        "¿Conoces la referencia del electrodoméstico?",
        options=["Sí", "No"],
        index=0 if caso_actual == "A" else 1,
        horizontal=True,
        key=f"op_126_caso_{clave}",
    )
    nuevo_caso = "A" if caso == "Sí" else "B"
    if nuevo_caso != caso_actual:
        datos["caso"] = nuevo_caso
        datos.pop("marca", None)
        datos.pop("referencia", None)
        datos.pop("alto", None)
        _registrar_edicion(clave, selecciones)
        st.rerun()
    ...
    if nuevo_caso == "A":
        ...
        referencia = st.text_input(
            "Referencia",
            value=datos.get("referencia", ""),
            key=f"op_126_ref_{clave}",
        )
        cambio = False
        if marca != datos.get("marca", ""):
            datos["marca"] = marca
            cambio = True
        if referencia != datos.get("referencia", ""):
        ...
        alto = st.text_input(
            "Altura del hueco (mm)",
            value=datos.get("alto", ""),
            key=f"op_126_alto_{clave}",
        )
        if alto != datos.get("alto", ""):
            datos["alto"] = alto
            _registrar_edicion(clave, selecciones)
            st.rerun()
...
def _renderizar_opcionales(
    ...
    """Renderiza checkboxes y radio. op_126 (bloque electro) va aparte tras divisor."""
            ...
            with st.expander(
                ...
                if aplicables:
                    ...
                    if "op_126" in aplicables:
                        st.divider()
                        _control_op_126(clave, estado["opcionales"], selecciones)
                else:
                    st.caption(
